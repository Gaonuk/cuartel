...
### D3. Sandbox is pluggable with two tiers (revised 2026-04-27)

**Decision:** Drop AgentOS secure-exec from the agent runtime. Ship `LocalSandbox` (claude-code-acp as plain host subprocess) as the MVP default. `AppleVzSandbox` / `HetznerSandbox` (real Linux VM) becomes the opt-in **secure mode** for autonomous / scheduled / unattended / remote work.

**Why:** A1 spike confirmed the V8-vs-OS hypothesis with one host-side run (no hang). For interactive sessions where the user is at the keyboard approving each tool call, the **user is the sandbox** — VM isolation is overkill and ships in every comparable product (Zed, Polyscope, Paseo, Cursor) without it. VM isolation is genuinely valuable for **autonomous work the user isn't watching** (cron Replicas, GitHub-webhook triggers, "ship it" autonomous mode, remote sessions on Hetzner) — and that's exactly when it ships, in Phase D.

**Original framing (real-VM-as-MVP) was over-rotated toward the security story.** Saved ~3 person-weeks by deferring; gets to dogfood faster; doesn't remove the security capability, just sequences it correctly.

**Concrete tech (unchanged from original D3, just resequenced):**
- MVP default — `LocalSandbox`: claude-code-acp as plain host subprocess. Phase B2.
- Local secure mode — `AppleVzSandbox` via Apple Virtualization.framework + shuru. Phase D step D0.
- Remote secure mode — `HetznerSandbox` via Firecracker / Cloud Hypervisor / KVM. Phase D step D1.
- Same VM image for D0 + D1.
...
22. **A1 spike executed + architectural pivot to host-direct MVP.** Spike A1 (claude-code-acp as plain Node OS process) ran successfully on first try: 21.9s, `stopReason: "end_turn"`, zero hang. Cost $0.115 (higher than estimated because the test prompt asked for a non-existent README, triggering 2 tool calls + agent reasoning). **One run sufficient** — the prior was very strong (Zed/Polyscope/Paseo/Cursor all run claude-code-acp as plain subprocess in production), and the hypothesis is binary (V8 nesting was/wasn't the cause). Skip the 50× run. **More importantly, the discussion that followed pivoted v2:** ship MVP with `LocalSandbox` only (no VM isolation), like every comparable product does for interactive sessions. `AppleVzSandbox` moves from Phase B2 (load-bearing MVP step) to Phase D step D0 (opt-in secure mode for autonomous/scheduled/unattended/remote work where the user isn't approving each tool call). Saves ~3 person-weeks; reduces Phase B from ~6.5 weeks to ~3.5 weeks; gets to dogfood faster. Decisions D3 in both v2 doc and KB §3 updated. **Concrete spike findings preserved in `spike/A1-localsandbox/A1-FINDINGS.md`** for B1 reference: `loadSession: true` (resolves open Q 3); tool calls are NOT surfaced via `session/update` notifications (only `agent_message_chunk` content) — implication for cuartel UI tool-call previews; two session-ID layers (ACP-level + Claude-internal); claude-code-acp logs to stdout mixed with JSON-RPC (parse-or-skip handles it); cost per turn ~$0.01-0.10 depending on tool-call activity.
